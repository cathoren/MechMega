                    ...
                    <nav className="hidden lg:flex justify-between items-center space-x-2">
                        {navigationItems.map((item) => (
                            <Link
                                key={item.href}
                                to={item.href}
                                className="relative px-4 py-2 text-sm 2xl:text-lg font-medium text-slate-200 hover:text-slate-900 dark:hover:text-white transition-all duration-300 rounded-lg hover:bg-slate-100/80 dark:hover:bg-slate-700/50 group">
                                {item.label}
